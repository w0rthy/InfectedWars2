...
function SWEP:ShootBullet(dmg, numbul, cone)
	local lcone = cone
	if self.Owner:Team()==0 and self.Owner:Alive() and self.Owner:GetUtility()==1 then lcone = math.max(lcone - 0.02,0) end
	local bullet = {}
	bullet.Num = numbul
	bullet.Src = self.Owner:GetShootPos()
	bullet.Dir = self.Owner:GetAimVector()
	bullet.Spread = Vector(lcone, lcone, 0)
	bullet.Tracer = 1
